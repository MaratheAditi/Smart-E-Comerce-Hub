...
def editProfile():
    ...
    with sqlite3.connect('database.db') as conn:
        cur = conn.cursor()
        cur.execute("SELECT userId, email, firstName, lastName FROM users WHERE email = ?", (session['email'], ))
...
def productDescription():
    ...
    conn.close()
    return render_template("productDescription.html", data=productData, my=my,loggedIn = loggedIn, firstName = firstName, noOfItems = noOfItems)


@app.route("/productDescription/create", methods=["GET", "POST"])
def create():
    if 'email' not in session:
        return redirect(url_for('loginForm'))
    else:
        ...
        if request.method == 'POST':    
            ...
            with sqlite3.connect('database.db') as conn:
                ...
                cur.execute('SELECT productId, name, price, description, image, stock FROM products WHERE productId = ?', (productId, ))
                productData = cur.fetchone()
                cur.execute('INSERT INTO review (name, content,rating,firstName, productId) VALUES (?, ?, ?, ?, ?)',( nav, text, ratings,firstName, productId))
                conn.commit()
            conn.close()
    ...
    if 'email' not in session:
        ...
        if request.method == 'POST':    
            ...
            with sqlite3.connect('database.db') as conn:
                ...
                firstName = cur.fetchone()[0]
                try:
                    cur.execute("select distinct productId from products where stock > 0")
...
def checkout():
    ...
    with sqlite3.connect('database.db') as conn:
            ...
            cur.execute("SELECT userId FROM users WHERE email = ?", (email, ))
            userId = cur.fetchone()[0]
            cur.execute("SELECT count(*) FROM kart")
            kart_count = cur.fetchone()[0]
            if kart_count > 0:
                flag = False
            cur.execute("SELECT products.productId,products.stock FROM products, kart WHERE products.productId = kart.productId AND kart.userId = ?", (userId, ))
            products = cur.fetchall()
            print("I am product ID",products)
            for ele in products:
                cur.execute("update products set stock=? where productId=?",(ele[1]-1,ele[0]))
                conn.commit()
            cur.execute("delete from kart")
            conn.commit()
    conn.close()
    if flag: return render_template("failure.html")
